...
def train(model, params, dataloader_label_train, dataloader_train, dataloader_audio_train,
    ...
    for epoch in range(max_num_epochs):
        for d, l, a in zip(dataloader_train.keys(), dataloader_label_train.keys(), dataloader_audio_train.keys()):
            ...
            # print(np.shape(img))
            # ===================forward=====================
            out, cnn_model, cnn_out, hidden_enc = model(img, target)
            if not target_bin:
...
def load_cnnseq2seq(use_best_checkpoint=True):
    ...
    model = load_model(cnn_params, cnnseq2seq_params, cnnseq2seq_model_path, saved_model, saved_model_cnn, device=device)
...
def load_model(cnn_params, cnnseq2seq_params, results_dir, saved_model_path, saved_model_path_cnn, device=torch.device('cpu')):

    # Load fine-tuned CNN with old parameters
    cnn_model = load_model_cnn(cnn_params, results_dir, saved_model_path_cnn)

    # Load CNNSeq2Seq
    model = CNNSeq2Seq(cnnseq2seq_params, cnn_model, device).cuda(cnnseq2seq_params['gpu_num'][0])
    model.load_state_dict(torch.load(results_dir + saved_model_path))
